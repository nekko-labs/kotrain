    ...
    return (
      <Suspense fallback={<div className="fixed inset-0 z-[60] bg-black" />}>
        <QrScanner
          ...
          onResult={(text) => {
            setScanning(false);
            if (!commit(text)) setError('That QR isn’t an Nekkos pairing code.');
          }}
        />
      </Suspense>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-6" style={{ background: 'var(--paper)' }}>
      ...
      <p className="mt-2 max-w-sm text-center text-[13px] text-ink-faint">
        Nekkos on your desktop → <span className="font-medium text-ink-soft">Settings → Remote access → Enable</span>, then paste the pairing link here. Your phone drives the model on your computer over an end-to-end encrypted relay.
      </p>
      <textarea
