...
export function AphelionAvatar({ size = 28, title }: { size?: number; title?: string }) {
  const C = APHELION;
  return (
    <svg viewBox="0 0 64 48" width={size} height={(size * 48) / 64} fill="none" role={title ? 'img' : 'presentation'} aria-label={title} aria-hidden={title ? undefined : true}>
...
export function Mascot({ mood, enabled }: { mood: MascotMood; enabled: boolean }) {
  ...
  return (
    <div className={`pointer-events-none fixed bottom-4 left-0 z-40 flex w-16 select-none items-end justify-center ${peek ? 'aphelion-peek' : ''}`}>
      <div className={`pointer-events-none md:pointer-events-auto md:cursor-pointer ${active ? 'aphelion-lean-right' : 'aphelion-lean'}`} onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)} onClick={() => setReacting(true)} title={thinking ? 'Aphelion is working…' : reacting ? 'Boost!' : 'Aphelion'} role="button" aria-label="Aphelion mascot">
        <div className={reactionAnim} onAnimationEnd={(e) => { if (e.target === e.currentTarget) setReacting(false); }}>
          <svg viewBox="-8 0 104 88" width="78" height="66" fill="none">
            ...
            <g className={bodyAnim} style={{ transformBox: 'fill-box', transformOrigin: 'center' }}>
              ...
              {!liftedFrontPaw && <SuitLeg C={C} d="M 48 59 C 48 66 48 72 49 77" boot={{ cx: 49, cy: 79 }} />}
