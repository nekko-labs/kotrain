// Produce a self-contained, publishable `nekko-paw` package under cli-dist/:
//   - index.mjs : the server + @nekko/host/core/shared bundled by esbuild
//   - web/      : the built renderer (the UI)
//   - package.json : name "nekko-paw", bin, and the few runtime deps
// Run after building the renderer (npm run build -w @nekko/desktop).
...
if (!existsSync(join(renderer, 'index.html'))) {
  console.error('Build the renderer first: npm run build -w @nekko/desktop');
  process.exit(1);
}
...
const pkg = {
  name: 'nekko-paw',
  version,
  description: 'Local-first AI coding & cowork — the self-hosted web edition. Run with `npx nekko-paw`.',
  license: 'MIT',
  type: 'module',
  bin: { 'nekko-paw': 'index.mjs' },
  files: ['index.mjs', 'web'],
  engines: { node: '>=20' },
  dependencies: {
...
writeFileSync(
  join(out, 'README.md'),
  '# Nekko Paw (web edition)\n\nRun:\n\n```bash\nnpx nekko-paw\n```\n\nThen open http://localhost:4317. See https://github.com/nekko-labs/nekko-paw\n',
);
