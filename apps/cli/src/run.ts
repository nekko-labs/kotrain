...
import type { AgentEvent } from '@nekkos/shared';
...
const HELP = `Nekkos CLI (nekkos ${VERSION}), drive your local agent from the terminal.

Usage:
  nekkos status [--json]              Show providers, model, workspaces, sessions
  nekkos sessions [--json]            List chat sessions
  nekkos chat "<prompt>" [opts]       Run an agent turn (streams the reply)
  nekkos watch [--session <id>]       Tail live agent events (best with --url)
  nekkos mcp                          Start the MCP server (stdio) for other tools
  nekkos --help | --version

Target (any command):
  --url <http://host:port>          Talk to a running server (else NEKKOS_URL,
                                    else the local data dir)
  --token <token>                   Bearer token for a secured server (NEKKOS_TOKEN)
...
Local data dir: ${dataDir()}  (override with NEKKOS_DATA_DIR)`;

/** Run a CLI invocation. Shared by the `nekkos` bin and the bundled `nekkos` package. */
export async function runCli(argv: string[]): Promise<void> {
  ...
  if (cmd === 'status') {
    ...
    console.log(`Nekkos, ${flags.url || process.env.NEKKOS_URL || dataDir()}`);
  ...
  if (cmd === 'chat') {
    const text = _[1];
    if (!text) throw new Error('Usage: nekkos chat "<prompt>"');
