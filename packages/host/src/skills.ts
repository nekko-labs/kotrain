...
import type { InstallTarget, InstallTargetInfo, InstalledSkillRecord, MarketplaceSkill } from '@nekkos/shared';
import { getMarketSkill, skillToMarkdown } from '@nekkos/shared';
import { dataDir } from './store.js';

/**
 * Skills marketplace installs. Records live in skills.json under the data dir.
 * The `nekkos` target is purely a record (installed skills join the `/` menu
...
export function skillTargets(): InstallTargetInfo[] {
  ...
  return [
    { id: 'nekkos', label: 'Nekkos', hint: 'joins the / menu and Skills tab', available: true },
    {
      id: 'claude',
      label: 'Claude Code',
