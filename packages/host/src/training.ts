...
function turnPrompt(run: TrainingRun, hints: string[]): string {
  ...
  if ((run.turns ?? 0) === 0) {
    if (run.kind === 'training') {
      parts.push(
        `You are Kotrain' data-scientist agent. Your job is to actually train a model for this purpose, working hands-on in the workspace (write code, run it with the bash tool, prefer Python; set up a venv or use available tooling; install packages as needed).`,
        `PURPOSE: ${run.goal}`,
      );
      const d = cfg.dataset;
      ...
      parts.push(
        `You are Kotrain' goal agent. You own a long-running goal and drive it to FINISHED, working hands-on in the workspace with your tools. You work in three phases: PLAN first, then EXECUTE, then ITERATE until it is genuinely done.`,
        `GOAL: ${run.goal}`,
